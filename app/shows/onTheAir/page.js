...
const page = () => {
  ...
  const onTheAirShows = async () => {
    try {
      const { data } = await axios.get("/3/tv/on_the_air?api_key=8c4692652155d837c43fac28f8dd958d");  
  ...
  if (onTheAir.length > 0) {
    showList = onTheAir.map((show, index) => {
      return (
  ...
  return (
    <div className='container mt-4 '>
      <h1 className='text-center'>On The Air Shows</h1>
